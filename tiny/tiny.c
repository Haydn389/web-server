...
int parse_uri(char *uri, char *filename, char *cgiargs)
{
  ...
  // 만일 요청이 정적 컨텐츠를 위한 것이라면  
  if(!strstr(uri, "cgi-bin")) //uri 중에 "cgi-bin"이 없으면 NULL 반환 -> if문 진입
  {
    // CGI 인자 스트링을 지우고
    strcpy(cgiargs, "");
...
void serve_dynamic(int fd, char *filename, char *cgiargs)
{
  ...
  */
  ...
  if(Fork() == 0)
  {
    ...
    // 다음으로, 자식은 자신의 표준 출력을 연결 파일 식별자로 재지정하고,
    Dup2(fd, STDOUT_FILENO); //fd를 STDOUT_FILENO(2)로 지정
